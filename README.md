...
* **팀 구성**: 👥 팀 프로젝트 / 9인
* **담당 업무**:
    * PPT 정보 분해 — 슬라이드별 텍스트·표·이미지 추출 및 스냅샷 생성 (`node_parse_all`)
    * 스크립트 품질 검토 & QnA 생성 — PASS/RETRY 조건부 분기 및 예상 QnA 자동 생성 (`node_review_script`)
* **사용 기술**:
...
- LibreOffice + poppler로 슬라이드 스냅샷(PNG) 생성 → 영상 배경으로 사용
### 🔍 외부 검색 보완
슬라이드 내용만으로 부족한 배경 지식을 실시간 웹 검색으로 자동 보완하여 강의 완성도를 높임
- Tavily Search API로 슬라이드 제목 기반 검색 수행
- 검색 결과를 내용 생성 단계에 함께 참조
### 📝 페이지별 내용 생성 & 강의 스크립트 작성
슬라이드의 텍스트·표·이미지를 Vision으로 종합 분석하고, 이전 페이지 흐름을 참조해 자연스러운 강의 대본 생성
- GPT-4o-mini Vision으로 멀티모달 슬라이드 분석
- 이전 페이지 요약을 맥락으로 전달하여 강의 흐름 일관성 유지
- 사용자 지정 톤·스타일 프롬프트 반영
### ✅ 스크립트 품질 검토 & QnA 생성
...
### 🔊 음성 합성 & 영상 제작
스크립트를 TTS로 음성 변환하고, 슬라이드 이미지와 합성하여 슬라이드별 영상을 만든 뒤 하나의 최종 영상으로 병합
- `gpt-4o-mini-tts`로 자연스러운 강의 음성 생성 (목소리 8종 선택)
- ffmpeg으로 슬라이드 이미지 + 음성 → MP4 합성 (1920×1080)
- 슬라이드별 영상을 하나의 최종 강의 영상으로 자동 병합
### 📄 강의 요약 보고서 PDF
전체 강의 내용을 사용자 지정 목적에 맞게 요약하여 PDF로 자동 출력
- 전체 스크립트 기반 요약 생성
- ReportLab으로 PDF 파일 자동 출력
### 🌐 Gradio 웹 UI
PPT 업로드부터 영상 재생·다운로드까지 별도 개발 없이 웹 인터페이스로 제공
- PPT 업로드, 강의 지침·목소리 선택 입력 UI
- 완성된 강의 영상 재생 및 다운로드
---
